...
ls -l $t.*
$ft o $t                                # 9) Order/optimize/pack
ls -l $t.*

echo "Testing get after order/pack"
for i in $*; do                         #10) Check everything is still there
  cmp <($ft g $t k$i) <(echo $i$d) >$n 2>&1 || echo mismatch4 at k$i
done
...
for i in $*; do                         #12) Check EVERYthing is still there
  cmp <($ft g $t k$i) <(echo $i$d) >$n 2>&1 || echo mismatch5 at k$i
done
cmp <($ft g $t K0) <(echo hellooooooooooo,there) >$n 2>&1 || echo K0 mismatch
